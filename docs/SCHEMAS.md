...
`serve-accountant-review-ui` starts the Streamlit review UI. It begins with source document upload/staging, then offers guided workflow run buttons for intake, document inventory, fact extraction, source matching, CoA/mapping preparation, review packet refresh, release candidate build, and final export. It also includes Source Extraction Review for incomplete extractions or wrong document-type candidates, accountant review decisions, artifacts, and controlled apply. The UI is experiential and workflow-friendly, but approvals still run through the same deterministic apply command.
