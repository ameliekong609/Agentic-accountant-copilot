...
`serve-accountant-review-ui` starts the Streamlit review UI. It begins with engagement setup/status and source document upload/staging, then offers guided workflow run buttons for intake, document inventory, fact extraction, source matching, CoA/mapping preparation, review packet refresh, reviewed TB/draft-statement build, release candidate build, and final export. It also includes Source Extraction Review for incomplete extractions or wrong document-type candidates with staged resolution actions, reviewed-trial-balance and internal-draft-statement screens, accountant review decisions with reviewer/rationale defaults and editable CoA rows, artifacts, final output review, and controlled apply. The UI is experiential and workflow-friendly, but approvals still run through the same deterministic apply command.
