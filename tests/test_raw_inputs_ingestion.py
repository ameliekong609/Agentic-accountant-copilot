...
from accountant_copilot.cli import (
    ...
    _pdf_password_candidates_from_filename,
...
def test_ingest_raw_inputs_suggests_display_name_for_uuid_bank_statement(tmp_path: Path):
    input_dir = tmp_path / "inputs"
    input_dir.mkdir()
    source = input_dir / "12C05E08-3847-4243-BE1F-7FDE142D977F.pdf"
    write_text_pdf(
        source,
        "Your Statement Account Number Statement Period 1 Jan 2025 - 31 Jan 2025 Closing Balance $30,325.01 CR "
        "Enquiries 13 1998 Business Transaction Account 027 If this account has an attached overdraft limit",
    )
    state = tmp_path / "state.json"
    write_state(state)
    ...
    assert result.returncode == 0
    document = json.loads(state.read_text())["source_documents"][0]
    assert document["original_file_name"] == "12C05E08-3847-4243-BE1F-7FDE142D977F.pdf"
    assert document["display_name"].startswith("2025-01-31 - Bank Statement")
    assert "attached" not in document["display_name"]
    assert "suggested_name" not in document
    assert document["naming_status"] == "suggested"
    assert document["naming_method"] == "deterministic"
    assert document["naming_evidence_refs"] == ["raw_001_page_001"]


def test_bank_statement_naming_includes_bank_and_stops_before_boilerplate():
    quote = (
        "Your Statement Account Number Statement Period 1 Jan 2025 - 31 Jan 2025 Closing Balance $30,325.01 CR "
        "Enquiries 13 1998 Business Transaction Account 027 If this account has an attached overdraft limit"
    )

    display_name, confidence, method = _name_from_quote(Path("source.pdf"), "bank_statement", quote)

    assert display_name == "2025-01-31 - Commonwealth Bank Statement - Account 027.pdf"
    assert confidence == "high"
    assert method == "deterministic"


def test_pdf_password_candidates_can_use_date_in_filename():
    assert _pdf_password_candidates_from_filename(Path("statement_password_08181986.pdf")) == ["08181986"]
